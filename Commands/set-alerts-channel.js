...
const { SlashCommandBuilder, Integration, IntegrationApplication, InteractionResponse , PermissionFlagsBits } = require('discord.js');
...
module.exports = {
    ...
    .addChannelOption(option =>
		option.setName('הערוץ-בו-תרצה-לקבל-התראות')
			.setDescription('הכנס את הערוץ שבו אתה רוצה לקבל את ההתראות'))
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),

    async execute(interaction) {
        const serverID = interaction.guild.id;
        const channelID = interaction.options.getChannel('הערוץ-בו-תרצה-לקבל-התראות').id;
