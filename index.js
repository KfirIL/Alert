...
client.on("ready", async () => {
  ...
  setInterval(async () => {
    ...
    .then((data) => {
  const givenDate = new Date('April 8, 2023 12:00:00');
          ...
          let channel = client.channels.cache.get(json[server].channel);
              ...
              data.forEach((alert) => {
                const embed = new EmbedBuilder()
                  ...
                  .addFields(
                    { name: `התראה של פיקוד העורף בשעה–${alert.time}`, value: alert.date },
                  );
...
client.on(Events.InteractionCreate, async interaction => {
	...
	try {
		...
		if (interaction.replied || interaction.deferred) {
			await interaction.followUp({ content: 'לא יכלנו לבצע את הפקודה! דווח על זה בבקשה בשרת הדיסקורד שלנו', ephemeral: true });
		} else {
			await interaction.reply({ content: 'לא יכלנו לבצע את הפקודה! דווח על זה בבקשה בשרת הדיסקורד שלנו', ephemeral: true });
		}
	}
});
